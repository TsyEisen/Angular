(function (angular) {
    var app = angular.module('addShareApp', ['ui.bootstrap']);
    app.controller('addShareController', ['$scope', '$state', '$window', '$uibModal', 'doctorService', 'institutionService', '$filter',
            ...
            $scope.ImgHighForm = function (isVaild) {
                if (isVaild) {
                    ...
                    if ($scope.startUploaderDate) {
                        message.uploadstarttime = doctorService.dataStrFormat($scope.startUploaderDate);
                    }
                    if ($scope.endUploaderDate) {
                        message.uploadendtime = doctorService.dataStrFormat($scope.endUploaderDate);
                    }
                    if ($scope.startScreenDate) {
                        message.studystarttime = doctorService.dataStrFormat($scope.startScreenDate);
                    }
                    if ($scope.endScreenDate) {
                        message.studyendtime = doctorService.dataStrFormat($scope.endScreenDate);
                    }
